...
namespace GiantSquid_Part1
{
    public class BingoBoard
    {
        ...
        {
            List<List<BingoBoardItem>> result = new();
        ...
        public int Sum(bool marked = false)
        {
            return Items.Sum((List<BingoBoardItem> row) => row
                .Where(item => item.Marked == marked)
                .Sum(item => item.Value));
        }
